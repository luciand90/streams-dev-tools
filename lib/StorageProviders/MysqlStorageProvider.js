...
var MysqlStorageProvider = function MysqlStorageProvider() {
    ...
    this.config.keyColumn = this.config.keyColumn || 'channelLabel';
    this.config.valueColumn = this.config.valueColumn || 'settings';
...
MysqlStorageProvider.prototype.replace = function(key, value, callback) {
    this.query(
...
MysqlStorageProvider.prototype.retrieve = function(key, callback) {
    this.query(
        ...
        function(err, records) {
            ...
            callback && callback(null, records[0] && JSON.parse(records[0].value));
        }
    );
};
